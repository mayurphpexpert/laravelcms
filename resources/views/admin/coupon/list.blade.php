            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids" class="checkbox_ids" id="" value="{{ $discountCoupon->id }}"></td>
                            <td>{{ $discountCoupon->id }}</td>
                            <td><a href="{{ route('coupons.edit', $discountCoupon->id) }}">{{ $discountCoupon->code }}</a></td>
                            <td><a href="{{ route('coupons.edit', $discountCoupon->id) }}">{{ $discountCoupon->name }}</a></td>
                            <td>
...
<script>
    function changePerPage(value) {
        window.location.href = "{{ route('coupons.index') }}" + "?per_page=" + value;
    }
    // Function to handle sorting by ID
    function sortById(order) {
        window.location.href = "{{ route('coupons.index') }}?sort=id&order=" + order;
    }
    ...
    function bulkDelete() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to delete selected Discount coupon?")) {
            var url = '{{ route("coupons.bulkDelete") }}';
            ...
            alert("Please select at least one Discount coupon to delete.");
        }
    }
    ...
    function bulkPublish() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to publish selected Discount coupon?")) {
            var url = '{{ route("coupons.bulkPublish") }}';

            sendBulkActionRequest(url, selectedIds);
        } else {
            alert("Please select at least one Discount coupon to publish.");
        }
    }
    ...
    function bulkUnpublish() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to unpublish selected Discount coupon?")) {
            var url = '{{ route("coupons.bulkUnpublish") }}';

            sendBulkActionRequest(url, selectedIds);
        } else {
            alert("Please select at least one Discount coupon to unpublish.");
        }
    }
