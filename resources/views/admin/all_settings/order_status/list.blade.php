    ...
    <div class="container-fluid my-2">
        <div class="row mb-2">
            ...
            <div class="col-sm-6 text-right">
                <a href="{{ route('orderStatus.create') }}" class="btn btn-primary">New Order Status</a>
            </div>
        </div>
    </div>
    <!-- /.container-fluid -->
            ...
            <form action="" method="get">
                <div class="card-header">
                    <div class="card-title">
                        <button type="button" onclick="window.location.href='{{ route("orderStatus.index") }}'" class="btn btn-default btn-sm">Reset</button>
                    </div>
                ...
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $orderStat->id }}</td>
                            <td><a href="{{ route('orderStatus.edit', $orderStat->id) }}">{{ $orderStat->name }}</a></td>
                            <td>{{ $orderStat->slug }}</td>
                            <td>
                            ...
                            <td>
                                <a href="{{ route('orderStatus.edit', $orderStat->id) }}">
                                    <svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                    </svg>
                                </a>

                                <a href="#" onclick="deletePage({{ $orderStat->id }})" class="text-danger w-4 h-4 mr-1">
...
<script>
    function changePerPage(value) {
        window.location.href = "{{ route('orderStatus.index') }}" + "?per_page=" + value;
    }
    // Function to handle sorting by ID
    function sortById(order) {
        window.location.href = "{{ route('orderStatus.index') }}?sort=id&order=" + order;
    }

    function deletePage(id) {
        var url = '{{ route("orderStatus.delete","ID") }}';
