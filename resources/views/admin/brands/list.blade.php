        ...
        <div class="card">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                    <thead>
                        <tr>
                            <th width="60"><input type="checkbox" name="" id="select_all_checkbox"></th>
                            <!-- <th width="60">ID</th> -->
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids" class="checkbox_ids" id="" value="{{ $brand->id }}"></td>
                            <td>{{ $brand->id }}</td>
                            <td>{{ $brand->name }}</td>
                            <td>{{ $brand->slug }}</td>
                            <td>
...
<script>
    ...
    function changePerPage(value) {
        window.location.href = "{{ route('brands.index') }}" + "?per_page=" + value;
    }
    // Function to handle sorting by ID
    function sortById(order) {
        window.location.href = "{{ route('brands.index') }}?sort=id&order=" + order;
    }
    ...
    function bulkDelete() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to delete selected brands?")) {
            var url = '{{ route("brands.bulkDelete") }}';
            ...
            alert("Please select at least one brands to delete.");
        }
    }
    ...
    function bulkPublish() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to publish selected brands?")) {
            var url = '{{ route("brands.bulkPublish") }}';

            sendBulkActionRequest(url, selectedIds);
        } else {
            alert("Please select at least one brands to publish.");
        }
    }
    ...
    function bulkUnpublish() {
        ...
        if (selectedIds.length > 0 && confirm("Are you sure you want to unpublish selected brands?")) {
            var url = '{{ route("brands.bulkUnpublish") }}';

            sendBulkActionRequest(url, selectedIds);
        } else {
            alert("Please select at least one brands to unpublish.");
        }
    }
